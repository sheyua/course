...
class NMT(Module):
    ...
    def __init__(self, embed_size: int, hidden_size: int, vocab: 'Vocab', dropout_rate: float=0.2) -> None:
        ...
        @param embed_size: Embedding size (dimensionality) -- e
        @param hidden_size: Hidden Size, the size of hidden states (dimensionality) -- h
        ...
        """
        from torch.nn import LSTM, LSTMCell, Linear, Dropout
    ...
    def forward(self, source: SentsType, target: SentsType) -> Tensor:
        ...
        @returns scores: a variable/tensor of shape (b, ) representing the log-likelihood of generating the
                         gold-standard target sentence for each example in the input batch. Here b = batch size.
        """
    ...
    def encode(self, source_padded: Tensor, source_lengths: List[int]) -> Tuple[Tensor, DecStateType]:
        ...
        @returns dec_init_state: Tuple of tensors representing the decoder's initial hidden state and cell.
        """
        from torch import allclose, transpose
        ...
        # # (1) use wide tensor inputs
        # assert not self.encoder.batch_first
        # o, (h_last, c_last) = self.encoder(input=X)
        # assert o.shape == (total, X.size(1), 2 * self.hidden_size)
        # assert h_last.shape == (2, X.size(1), self.hidden_size)
        # assert c_last.shape == (2, X.size(1), self.hidden_size)
        # hf_last, hb_last = h_last[0, :, :], h_last[-1, :, :]
        # cf_last, cb_last = c_last[0, :, :], c_last[-1, :, :]
        # assert allclose(o[0, :, self.hidden_size:], hb_last)
        # assert allclose(o[-1, :, :self.hidden_size], hf_last)
        # enc_hiddens = transpose(input=o, dim0=0, dim1=1)

        # (2) use pack inputs to cut the hidden/cell at sentence end
        ...
        # o, lengths = pad_packed_sequence(sequence=o_, batch_first=False, padding_value=0., total_length=total)
        # assert o.shape == (total, X.size(1), 2 * self.hidden_size)
        # assert h_last.shape == (2, X.size(1), self.hidden_size)
        # assert c_last.shape == (2, X.size(1), self.hidden_size)
        # hf_last, hb_last = h_last[0, :, :], h_last[-1, :, :]
        ...
        #     assert size == source_lengths[idx]
        #     assert allclose(o[size - 1, idx, :self.hidden_size], hf_last[idx, :])
        # enc_hiddens = transpose(input=o, dim0=0, dim1=1)

        # (3) use pad_packed_sequence to transpose rather than transpose in (2)
        enc_hiddens, lengths = pad_packed_sequence(sequence=o_, batch_first=True, padding_value=0., total_length=total)
        # assert enc_hiddens.shape == (X.size(1), total, 2 * self.hidden_size)
        # assert h_last.shape == (2, X.size(1), self.hidden_size)
        # assert c_last.shape == (2, X.size(1), self.hidden_size)
        # hf_last, hb_last = h_last[0, :, :], h_last[-1, :, :]
        # cf_last, cb_last = c_last[0, :, :], c_last[-1, :, :]
        # assert allclose(enc_hiddens[:, 0, self.hidden_size:], hb_last)
        # for idx, size in enumerate(lengths):
        #     assert size == source_lengths[idx]
        #     assert allclose(enc_hiddens[idx, size - 1, :self.hidden_size], hf_last[idx, :])

        init_decoder_hidden = transpose(input=h_last, dim0=0, dim1=1).reshape([-1, 2 * self.hidden_size])
        init_decoder_hidden = self.h_projection(input=init_decoder_hidden)
        init_decoder_cell = transpose(input=c_last, dim0=0, dim1=1).reshape([-1, 2 * self.hidden_size])
    ...
    def decode(self, enc_hiddens: Tensor, enc_masks: Tensor, dec_init_state: DecStateType,
        ...
        o_prev = zeros(batch_size, self.hidden_size, device=self.device)
    ...
    def step(self, Ybar_t: Tensor, dec_state: DecStateType, enc_hiddens: Tensor, enc_hiddens_proj: Tensor,
             enc_masks: Tensor) -> Tuple[DecStateType, Tensor, Tensor]:
    ...
    def generate_sent_masks(self, enc_hiddens: Tensor, source_lengths: List[int]) -> Tensor:
        ...
        """
        from torch import zeros, float

        enc_masks = zeros(enc_hiddens.size(0), enc_hiddens.size(1), dtype=float)
    ...
    def beam_search(self, src_sent: SentType, beam_size: int=5, max_decoding_time_step: int=70) -> List[Hypothesis]:
        ...
        """
        from torch.nn.functional import log_softmax
        from torch import zeros, float, long, tensor, cat, topk
        ...
        h_tm1 = dec_init_vec
        att_tm1 = zeros(1, self.hidden_size, device=self.device)

        eos_id = self.vocab.tgt['</s>']

        hypotheses = [['<s>']]
        hyp_scores = zeros(len(hypotheses), dtype=float, device=self.device)
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            y_tm1 = tensor([self.vocab.tgt[hyp[-1]] for hyp in hypotheses], dtype=long, device=self.device)
            ...
            live_hyp_ids = tensor(live_hyp_ids, dtype=long, device=self.device)
            h_tm1 = (h_t[live_hyp_ids], cell_t[live_hyp_ids])
            att_tm1 = att_t[live_hyp_ids]

            hypotheses = new_hypotheses
            hyp_scores = tensor(new_hyp_scores, dtype=float, device=self.device)
    ...
    @property
    def dtype(self) -> type: return self.model_embeddings.source.weight.dtype
    ...
    def save(self, path: str) -> None:
        ...
        print('save model parameters to [%s]' % path, file=stderr)
