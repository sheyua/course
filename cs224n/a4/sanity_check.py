...
from utils import SentsType, batch_iter
...
def question_1d_sanity_check(model: NMT, src_sents: SentsType, tgt_sents: SentsType, vocab: Vocab) -> None:
    ...
    # Load Outputs
    enc_hiddens_target = torch.load(abspath(f'{dirname(__file__)}/sanity_check_en_es_data/enc_hiddens.pkl'))
    dec_init_state_target = torch.load(abspath(f'{dirname(__file__)}/sanity_check_en_es_data/dec_init_state.pkl'))
...
def question_1e_sanity_check(model: NMT, src_sents: SentsType, tgt_sents: SentsType, vocab: Vocab) -> None:
    ...
    # Load Inputs
    dec_init_state = torch.load(abspath(f'{dirname(__file__)}/sanity_check_en_es_data/dec_init_state.pkl'))
    enc_hiddens = torch.load(abspath(f'{dirname(__file__)}/sanity_check_en_es_data/enc_hiddens.pkl'))
    enc_masks = torch.load(abspath(f'{dirname(__file__)}/sanity_check_en_es_data/enc_masks.pkl'))
    target_padded = torch.load(abspath(f'{dirname(__file__)}/sanity_check_en_es_data/target_padded.pkl'))

    # Load Outputs
    combined_outputs_target = torch.load(abspath(f'{dirname(__file__)}/sanity_check_en_es_data/combined_outputs.pkl'))
    ...
    def stepFunction(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks):
       dec_state = torch.load(abspath(f'{dirname(__file__)}/sanity_check_en_es_data/step_dec_state_{COUNTER[0]}.pkl'))
       o_t = torch.load(abspath(f'{dirname(__file__)}/sanity_check_en_es_data/step_o_t_{COUNTER[0]}.pkl'))
       COUNTER[0] += 1
...
def question_1f_sanity_check(model: NMT, src_sents: SentsType, tgt_sens: SentsType, vocab: Vocab) -> None:
    ...
    # Inputs
    Ybar_t = torch.load(abspath(f'{dirname(__file__)}/sanity_check_en_es_data/Ybar_t.pkl'))
    dec_init_state = torch.load(abspath(f'{dirname(__file__)}/sanity_check_en_es_data/dec_init_state.pkl'))
    enc_hiddens = torch.load(abspath(f'{dirname(__file__)}/sanity_check_en_es_data/enc_hiddens.pkl'))
    enc_masks = torch.load(abspath(f'{dirname(__file__)}/sanity_check_en_es_data/enc_masks.pkl'))
    enc_hiddens_proj = torch.load(abspath(f'{dirname(__file__)}/sanity_check_en_es_data/enc_hiddens_proj.pkl'))

    # Output
    dec_state_target = torch.load(abspath(f'{dirname(__file__)}/sanity_check_en_es_data/dec_state.pkl'))
    o_t_target = torch.load(abspath(f'{dirname(__file__)}/sanity_check_en_es_data/o_t.pkl'))
    e_t_target = torch.load(abspath(f'{dirname(__file__)}/sanity_check_en_es_data/e_t.pkl'))
...
def sanity_read_corpus(file_path: str, source: str) -> SentsType:
    ...
    data = []
    for line in open(abspath(f'{dirname(__file__)}/{file_path}')):
